...
export const Services = () => {
  ...
  const services = [
    {
      icon: <Video className="text-foreground" size={32} />,
    ...
    {
      icon: <Zap className="text-foreground" size={32} />,
      ...
      <section id="services" className="py-12 sm:py-16 lg:py-20 px-4 sm:px-6 lg:px-8 bg-background">
        <div className="max-w-7xl mx-auto">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-4 sm:mb-6 text-foreground">My Services & Rates</h2>
          ...
          <div className="grid lg:grid-cols-3 gap-6 sm:gap-8">
            {services.map((service, index) => (
              <motion.div
                ...
                className="h-full"
              >
                <Card className="bg-card border border-border hover:border-foreground transition-all duration-300 h-full flex flex-col hover:shadow-lg">
                  <CardContent className="p-6 sm:p-8 flex flex-col h-full">
                    <div className="flex items-center mb-4 sm:mb-6">
                      {service.icon}
                      <div className="ml-4">
                        <h3 className="text-xl sm:text-2xl font-bold text-foreground">{service.title}</h3>
                        <p className="text-foreground font-semibold text-lg sm:text-xl">{service.price}</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3 sm:space-y-4 mb-6 sm:mb-8">
                      {service.features.map((feature, featureIndex) => (
                        <div key={featureIndex} className="flex items-start">
                          <Check className="text-foreground mr-3 mt-0.5 flex-shrink-0" size={16} />
                    ...
                    <Button 
                      ...
                      className="w-full border border-foreground text-foreground hover:bg-foreground hover:text-background font-semibold mt-auto transition-all duration-300"
                      variant="outline"
                    >
                      Get Started
                    </Button>
